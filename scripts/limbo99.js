import { fromHexString, randomNumberFromInputs, uint64FromString } from "./utils/deterministic.js";

function calculateMultiplier(result) {
  const RTP = 0.99;
  const MIN = 0;
  const MAX = 1;
  if (result < MIN || result > MAX) throw new Error(`Prediction must be between ${MIN} and ${MAX} (inclusive)`);
  const houseEdge = 1 - RTP;
  const probability = (MAX - result) / (MAX - MIN);
...
document.getElementById("generateBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    const nonce = uint64FromString(nonceValue);
    const clientSecret = fromHexString(clientSecretValue);
    const rnd = await randomNumberFromInputs([inputHash, clientSecret, nonce], 1, "concat");
    resultEl.value = calculateMultiplier(rnd);
  } catch (error) {
